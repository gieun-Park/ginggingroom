...
- 📸 **이미지 업로드** - 컴퓨터에서 사진 선택
- 📷 **카메라 촬영** - 웹캠으로 실시간 촬영
- ⚪ **자동 흰색 배경** - 업로드한 사진과 카메라 촬영본의 실제 배경을 자동으로 흰색으로 교체
...
├── 📁 js/
│   ├── 📄 app.js              # 메인 애플리케이션 로직
│   ├── 📄 background-composite.js # 인물 마스크와 흰색 배경 합성
│   ├── 📄 background-segmentation.js # 브라우저 인물 분할 모델 실행
│   ├── 📄 background-session.js # 사진별 배경 분석 상태 관리
...
│       └── selfie_segmenter.tflite # 로컬 인물 분할 모델
│
└── 📁 tests/                  # 자동화 테스트
```
...
사진을 업로드하거나 카메라로 촬영하면 SelfieSegmenter가 사진 속 환경을 흰색으로 교체하려고 자동 분석합니다.
SelfieSegmenter가 인물로 유지한 모든 사람은 합성 결과에도 함께 남습니다.
프레임을 변경할 때는 같은 사진의 얼굴 분석과 배경 분석 결과를 모두 재사용하므로 다시 분석하지 않습니다.

인물 분할에 실패하면 원본 사진으로 안전하게 돌아가 프레임 합성을 계속 표시하며, 화면의 **배경 다시 시도** 버튼으로 얼굴 분석과 별개로 배경 분석만 다시 시도할 수 있습니다.
사진, 얼굴 랜드마크, 인물 마스크, 전경 Canvas는 모두 브라우저 안에만 머물며 서버로 전송되지 않습니다.
